  ...
  expressAsyncHandler(async (req, res) => {
    ...
    if (user) {
      if (user.email === 'admin@example.com') {
        res
          .status(400)
          .send({ message: 'Nie można usunąć konta głównego admina!' });
        return;
      }
      await user.remove();
      res.send({ message: 'Użytkownik usunięty!' });
    } else {
      res.status(404).send({ message: 'Użytkownik nieznaleziony!' });
    }
  })
);
userRouter.post(
  ...
  expressAsyncHandler(async (req, res) => {
    ...
    res.status(401).send({ message: 'Nieprawidłowe hasło lub email!' });
  })
);
